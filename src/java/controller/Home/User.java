    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equals("updateinfo")) {
            try {
                ...
                if (user != null) {
                    ...
                    model.User user1 = new model.User(user.getUser_id(), user_name, user.getUser_email(), user_pass, user.getIsAdmin());
        ...
        if (action.equals("signup")) {
            ...
            if (!user_pass.equals(re_pass)) {
                ...
                UserDAO dao = new UserDAO();
                model.User a = dao.checkAcc(user_email);
                if (a == null) {
                    dao.signUp(user_email, user_pass);
                    request.setAttribute("done", "Đăng kí thành công!");
                    request.getRequestDispatcher("user?action=login").forward(request, response);
                } else {
